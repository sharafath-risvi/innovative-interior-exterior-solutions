...
export default function StorytellingSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      mm.add("(max-width: 767px)", () => {
        // Reset any GSAP inline styles that might interfere on mobile
  ...
  return (
    ...
    >
      {/* ── WRAPPER ── */}
      <div ref={pinRef} className="w-full md:h-screen md:overflow-hidden md:flex md:items-center md:justify-center bg-[#0a0a0a] relative z-10 pt-32 pb-24 md:py-0">
        ...
        <div className="absolute top-8 md:top-12 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
          <span className="text-[10px] md:text-xs font-semibold tracking-[0.3em] uppercase px-6 py-2 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-md text-white/50 whitespace-nowrap">
            Design Journey
          </span>
        </div>
        ...
        {/* ================================================= */}
        <div className="relative w-full max-w-7xl h-full flex flex-col md:flex-row md:items-center md:justify-center perspective-[1500px] transform-gpu px-4 md:px-8 gap-12 md:gap-0 mt-8 md:mt-0 mx-auto">
...
const JourneyCard = forwardRef(({ phase, title, subtitle, desc, img, zIndex }, ref) => (
  <div 
    ref={ref} 
    className="relative md:absolute w-full md:w-[90%] max-w-4xl bg-[#141414] border border-white/10 p-6 md:p-10 rounded-3xl shadow-2xl flex flex-col md:flex-row gap-6 md:gap-8 items-center md:will-change-transform"
    style={{ zIndex }}
  >
    <div className="w-full md:w-1/2 rounded-2xl overflow-hidden h-64 md:h-[400px] shrink-0 border border-white/5 relative group">
